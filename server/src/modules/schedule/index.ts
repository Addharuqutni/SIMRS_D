...
// GET display queues
router.get('/queues/display', requireAuth, asyncHandler(async (req, res) => {
...
router.post('/queues/next', requireAuth, asyncHandler(async (req, res) => {
    ...
    // Real-time broadcast: display boards & loket update instantly (no polling)
    emitQueueCalled(poliId, queueToCall.queueCode || String(queueToCall.queueNumber), queueToCall.loket || undefined);
    emitQueueUpdate(poliId, { sedangDilayani: queueToCall.queueCode, calledAt: new Date().toISOString() });
